...
<%!String info = "/home/siena/workspace/AVA/WebContent/data/ghramos/info.json";
	...
	String getValorDisciplina(String key) throws FileNotFoundException, IOException, ParseException {
		String str = "";
		Object obj = parser.parse(new FileReader(disciplinas));
		JSONObject jsonObj = (JSONObject) obj;
		JSONArray disciplinas = (JSONArray) jsonObj.get("disciplinas");
		str += "<div class=\"box-header with-border\">Disciplina</div>";
		str += "<div class=\"box-body\">";
		for (int i = 0; i < disciplinas.size(); i++) {
			JSONObject o = (JSONObject) disciplinas.get(i);
			String t = (String) o.get("nome");
			if (t.equalsIgnoreCase(key)) {
				str += "<h2><p>" + o.get("descricao") + "</p></h2>";
				str += "<p>Professor(a): " + o.get("professor") + "</p>";
			}
		}
		str += "</div>";
		return str;
	...
	String getAlunosDisciplina(String key) throws FileNotFoundException, IOException, ParseException {
		String str = "";
		Object obj = parser.parse(new FileReader(disciplinas));
		JSONObject jsonObj = (JSONObject) obj;
		JSONArray disciplinas = (JSONArray) jsonObj.get("disciplinas");
		str += "<div class=\"box-header with-border\">Alunos</div>";
		str += "<div class=\"box-body\">";
		str += "<ul>";
		for (int i = 0; i < disciplinas.size(); i++) {
			JSONObject o = (JSONObject) disciplinas.get(i);
	...
	<div class="main" id="myMain">
		<div class="main-center">
			...
			<div class="box box-info">
				<%
					if (request.getParameter("id") != null) {
						out.println(getValorDisciplina(request.getParameter("id")));
					}
				%>
			</div>
			<!-- ALUNOS -->
			<div class="box box-info">
				<%
					if (request.getParameter("id") != null) {
						out.println(getAlunosDisciplina(request.getParameter("id")));
					}
				%>
